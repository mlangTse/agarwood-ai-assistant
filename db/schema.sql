...
create table if not exists incense_regions (
  id uuid primary key default gen_random_uuid(),
...
create table if not exists scent_tags (
  id uuid primary key default gen_random_uuid(),
  name text not null unique,
  description text,
  created_at timestamptz not null default now()
...
create table if not exists products (
  ...
  region_id uuid references incense_regions(id) on delete set null,
  ...
  aroma_profile_id uuid references aroma_profiles(id) on delete set null,
...
create index if not exists products_budget_level_idx on products(budget_level);
...
insert into incense_regions (name, country, aroma_character, typical_scenes)
values
